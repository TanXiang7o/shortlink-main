...
import com.nageoffer.shortlink.project.mq.producer.mqProducer;
...
public class ShortLinkServiceImpl extends ServiceImpl<ShortLinkMapper, ShortLinkDO> implements ShortLinkService {
    ...
    @Value("${shortlink.stats.queue}")
    private String shortlinkStatsQueue;
...
//                .favicon(getFavicon(requestParam.getOriginUrl()))
    ...
    public void restoreUrl(String shortUri, ServletRequest request, ServletResponse response) {
        ...
        //如果redis查不到，再查布隆过滤器，还是没有，就是没有这个短链接
        ...
        try {
            ...
            //这里逻辑是先查路由表找到gid，再根据Gid和fullshorturl查ShortlinkDO，个人感觉没必要查gid
            LambdaQueryWrapper<ShortLinkGotoDO> linkGotoQueryWrapper = Wrappers.lambdaQuery(ShortLinkGotoDO.class)
    ...
    public void shortLinkStats(String fullShortUrl, String gid, ShortLinkStatsRecordDTO statsRecord) {
        ...
        if(StrUtil.equals(shortlinkStatsQueue,"redisStream")){
            System.out.println("使用redisStream作为消息队列");
            shortLinkStatsSaveProducer.send(producerMap);
        } else if(StrUtil.equals(shortlinkStatsQueue,"rocketmq")){
            System.out.println("使用rocketmq作为消息队列");
            mqProducer.send(producerMap);
        }
    }
